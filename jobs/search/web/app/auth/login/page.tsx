...
export default function LoginPage() {
  ...
  const verifyOtp = async (e: React.FormEvent) => {
    ...
    setError(''); setLoading(true)
    try {
      const data = await authApi.verifyOtp({ phone_number: phone, code: otp, purpose: 'LOGIN' })
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8 w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900">{t('Welcome back', 'Karibu tena')}</h1>
          <p className="text-gray-500 text-sm mt-1">{t('Login to your Search account', 'Ingia kwenye akaunti yako ya Search')}</p>
        </div>
        ...
        {step === 'phone' ? (
          <form onSubmit={requestOtp} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-emerald-400 focus:ring-1 focus:ring-emerald-400"
                required
              />
              <p className="text-xs text-gray-400 mt-1">{t('We\'ll send you a 6-digit code', 'Tutakutumia namba ya tarakimu 6')}</p>
            </div>
            ...
            <button type="submit" disabled={loading} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-semibold hover:bg-emerald-700 disabled:opacity-50 transition-colors">
              {loading ? t('Sending...', 'Inatuma...') : t('Send OTP', 'Tuma Namba')}
            </button>
          </form>
        ) : (
          <form onSubmit={verifyOtp} className="space-y-4">
            ...
            <button type="submit" disabled={loading || otp.length !== 6} className="w-full bg-emerald-600 text-white py-3 rounded-xl font-semibold hover:bg-emerald-700 disabled:opacity-50 transition-colors">
              {loading ? t('Verifying...', 'Inathibitisha...') : t('Verify & Login', 'Thibitisha & Ingia')}
            </button>
            <button type="button" onClick={() => { setStep('phone'); setOtp(''); setError('') }} className="w-full text-sm text-gray-500 hover:underline">
              {t('Change phone number', 'Badilisha namba ya simu')}
            </button>
          </form>
        )}
        ...
        <p className="text-center text-sm text-gray-500 mt-6">
          ...
          <Link href="/auth/register" className="text-emerald-600 font-medium hover:underline">{t('Register', 'Jisajili')}</Link>
        </p>
      </div>
    </div>
  )
