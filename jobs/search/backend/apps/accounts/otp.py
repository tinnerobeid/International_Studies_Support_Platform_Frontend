...
def send_otp(phone_number, purpose='LOGIN'):
    ...
    if settings.MOCK_OTP:
        logger.warning(f"[MOCK OTP] {phone_number} -> {code} ({purpose})")
...
def _send_via_africastalking(phone_number, code):
    try:
        ...
        msg = f"Msimbo wako wa Search ni: {code}. Utaisha dakika {settings.OTP_EXPIRY_MINUTES}. Usishirikishe mtu."
        africastalking.SMS.send(msg, [phone_number])
